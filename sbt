...
declare -a myargs=('sbt' '-v' '-sbt-snapshot' '-210' '-Djava.awt.headless=true' '-Dfile.encoding=UTF8' '-mem' '4096' '-J-Xss10M' '-J-XX:+CMSClassUnloadingEnabled' '-J-XX:-UseGCOverheadLimit');
...
echoerr () {
  [[ -z $quiet ]] && echo 1>&2 "$myargs"
}
vlog () {
  [[ $verbose || $debug ]] && echoerr "$myargs"
}
dlog () {
  [[ $debug ]] && echoerr "$myargs"
}
